...
final class TemplateController extends AbstractController
{
    ...
    public function fillFileBulk(
        ...
        $langOverride = trim((string) ($data['language'] ?? $data['lang'] ?? ''));
        if ($langOverride !== '') {
            $companyData['language'] = $langOverride;
        }
        ...
        foreach ($templates as $tplPath) {
            ...
            if (str_contains($tplPath, '__zip_templates_test_')) {
                $results[] = [
                    'template' => $tplPath,
                    'status'   => 'FAIL',
                    'error'    => 'Tai PHPUnit testų likutis (katalogas __zip_templates_test_*). Ištrinkite jį iš templates/ arba naudokite tikrus šablonus.',
                ];
                continue;
            }

            if (str_contains($tplPath, '__tests_update')) {
                $results[] = [
                    'template' => $tplPath,
                    'status'   => 'FAIL',
                    'error'    => 'Tai PHPUnit testų likutis (katalogas __tests_update). Ištrinkite templates/__tests_update arba naudokite tikrus šablonus.',
                ];
                continue;
            }
    ...
    private function filterTemplatesOnly(array $items): array
    {
        ...
        foreach ($items as $item) {
            if ($item['type'] === 'directory') {
                $dirName = (string) ($item['name'] ?? '');
                if (str_starts_with($dirName, '__zip_templates_test_')
                    || str_starts_with($dirName, '__tests_update')) {
                    continue;
                }
                $result[] = [
                    'name'     => $item['name'],
