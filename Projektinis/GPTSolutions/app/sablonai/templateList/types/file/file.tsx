...
import { clearDeletedCaches, clearTemplatesAndGeneratedCaches } from "@/lib/cache/catalogueTreeCache";
...
export default function Files({ data, fileType }: List) {
  ...
  async function restoreFile() {
    FilesApi.restoreFile(data.path).then((res) => {
      if (res.status === "SUCCESS") {
        DirectoryStore.remove(data.path);
        clearTemplatesAndGeneratedCaches();
        setCatalogueTree((prev) => removeFileFromTree(prev, data.path));
      }
    })
  }
  ...
  async function downloadFile() {
    const { blob, filename } = await FilesApi.downloadFile(`${fileType}/${data.path}`);
