...
export default function Page({ params }: PageProps) {
  ...
  const directory = Array.isArray(directorySeg)
    ? directorySeg
    : directorySeg != null
      ? [directorySeg]
      : [];
  ...
  const [customVariables, setCustomVariables] = useState<CustomVariable | null>(
    null,
  );
  ...
  async function updateDocument() {
    ...
    const cleanedCustomVariables = Object.fromEntries(
      Object.entries(customVariables || {}).filter(
        ([, value]) => value.trim() !== "",
      ),
    );

    console.log("cleanedCustomVariables", cleanedCustomVariables);

    const res = await TemplateApi.createDocument(
      Number(companyId),
      [templatePath],
    ...
    if (res) {
      const fileRes = await FilesApi.getFileData(root, fullPath);
      setModifiedDate(fileRes.metadata?.custom?.modifiedAt || "");
      setUser(currentUser);
    }
  }

  async function downloadDocument() {
    GeneratedFilesApi.getGeneratedWord(fullPath).then((res) =>
      downloadBlob(res),
    );
  }
      ...
      <div className={style.pageColumn}>
        ...
        <div className={style.wrapper}>
          <div className={style.main}>
            ...
            {company?.modifiedAt &&
              modifiedDate &&
              new Date(modifiedDate.replace(" ", "T")) <
                new Date(company.modifiedAt.replace(" ", "T")) && (
