...
final class GetPDF
{
    ...
    public function convertToPdf(string $relativePath): string
    {
        ...
        if ($filePath === false || !str_starts_with($filePath, realpath($templatesDir))) {
            throw new \InvalidArgumentException('Failas nerastas arba kelias išeina iš templates ribų');
        }

        if (!is_readable($filePath)) {
            throw new \InvalidArgumentException('Failas neprieinamas: ' . $relativePath);
        }

        $ext = strtolower(pathinfo($filePath, PATHINFO_EXTENSION));
        if (!in_array($ext, ['doc', 'docx'], true)) {
            throw new \InvalidArgumentException('Palaikomi tik .doc ir .docx formatai');
        }
        ...
        if ($exitCode !== 0) {
            throw new \RuntimeException('PDF konvertavimas nepavyko: ' . implode("\n", $output));
        }
        ...
        if (!file_exists($pdfPath)) {
            throw new \RuntimeException('PDF failas nesugeneruotas');
        }
